...
running editor** at `http://127.0.0.1:8000/mcp`. In Codex it is registered as an
HTTP-transport server — **typically `unreal-editor`**, so the meta-tools appear as
`mcp__unreal-editor__*`. If `Codex mcp list` shows a different name, substitute it
...
```bash
Codex mcp list        # expect e.g.:  unreal-editor: http://127.0.0.1:8000/mcp (HTTP) - ✔ Connected
```

- **If it's Connected** → go to step 2.
- **If it's missing/disconnected**, the editor isn't running OR it started after this
  Codex session. The MCP client connects only at **session start**, so:
  ...
  2. Wait until the server answers (editor load takes a minute):
     ...
     ```
  3. **Restart Codex** so the `mcp__unreal-editor__*` tools load. (Tools only
